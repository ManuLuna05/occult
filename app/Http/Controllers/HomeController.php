...
class HomeController extends Controller
{
    ...
    private function getPlayers()
    {
        return [
            ...
            [
                ...
                'country' => 'España',
                'foot'    => 'Derecha',
                'bio'     => 'Lateral con gran visión de juego para la salida de balón, destaca por su capacidad para incorporarse al ataque y su precisión en los centros. Su colocación y resistencia le permiten defender toda la banda, siendo un jugador clave en la creación de oportunidades de gol en recuperaciones altas. A lo largo de la temporada ha demostrado una gran regularidad, aportando asistencias importantes y contribuyendo en defensa con su entrega y sacrificio. Su versatilidad le permite adaptarse a diferentes roles dentro del equipo, siendo una pieza fundamental en el esquema táctico del entrenador.',
    ...
    public function calendario(Request $request)
    {
        ...
        $season = $request->query('season', 'current');
        if (!in_array($season, ['current', 'previous'], true)) {
            $season = 'current';
        }

        $seasonMeta = [
            'current' => [
                'title' => 'Calendario - 2º Split',
                'subtitle' => 'Temporada 2025/26',
            ],
            'previous' => [
                'title' => 'Calendario - 1º Split',
                'subtitle' => 'Temporada 2025/26',
            ],
        ];

        $matches = $season === 'previous'
            ? $this->getMatchesPrevious()
            : $this->getMatchesCurrent();

        $matchesByMonth = collect($matches)->groupBy('month');

        return view('calendario', compact('matchesByMonth', 'season', 'seasonMeta'));
    }

    // Mantiene compatibilidad con la home y cualquier otra vista antigua
    private function getMatches()
    {
        return $this->getMatchesCurrent();
    }

    // Renombra tu getMatches() actual a esto:
    private function getMatchesCurrent()
    {
        return [
            [
                'month' => 'Enero',
    ...
    // Añade este método (rellénalo con tus partidos de la temporada pasada)
    private function getMatchesPrevious()
    {
        return [
